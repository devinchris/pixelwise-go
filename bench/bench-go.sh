#!/usr/bin/env bash
# Benchmark script for the Go (Fiber v2) implementation.
#
...
readonly SERVICE="pixelwise-go"
readonly COMPETING_SERVICE="pixelwise-python"
...
for USE_DB_VALUE in false true; do
  ...
  for C in "${CONCURRENCY_LEVELS[@]}"; do
    echo "Running: Go | c=${C} | USE_DB=${USE_DB_VALUE}"

    {
      echo ""
      echo "========================================================"
      echo " Go | concurrency=${C} | USE_DB=${USE_DB_VALUE}"
      echo "========================================================"
    } >> "${OUTFILE}"
